"""
G0 — Carga el texto del corpus en la BD con pgvector, por chunks, y mide el tiempo de cada chunk.

- Idéntico a P0: el texto no es un vector, así que pgvector no cambia nada aquí. Sirve para
  tener las frases en la BD de pgvector y comprobar que la carga de texto cuesta lo mismo.
- 1 línea = 1 frase; id = número de línea (desde 0), el mismo en todos los sistemas.
...
pgvector/docker-compose.yml: puerto 5434, BD lab_pgvector).

Uso:  python pgvector/G0_load_text.py [--corpus F] [--out-dir D] [--sizes 1 10 ...]
"""
...
def main():
    ...
    for size in args.sizes:
        ...
        for n, start in enumerate(range(0, len(records), size)):
            ...
            db_calls += 2
            rows.append(["pgvector", size, "insert_texto", "-", n, 0, f"{times[-1]:.9f}"])
    ...
    out = args.out_dir / "tiempos_g0.csv"
